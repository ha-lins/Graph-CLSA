...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
            ...
            cleaned=False, aug=None, stro_aug=None):
        self.name = name
...
def ppr_aug(data):
    # print('before get_adj :{}'.format(datetime.datetime.now()))
    adj_matrix = get_adj_matrix(data)
    # obtain exact PPR matrix
    # print('before get_ppr :{}'.format(datetime.datetime.now()))
    ppr_matrix = get_ppr_matrix(adj_matrix)
    # print('after get_ppr :{}'.format(datetime.datetime.now()))
    ...
    if k:
        ...
        raise ValueError
    # print('before get_edge :{}'.format(datetime.datetime.now()))
...
def drop_nodes(data):
    ...
    # print('[info] edge_index is:{}'.format(data.edge_index))
    ...
    data.edge_index = edge_index
    # print('[info] edge_index is:{}'.format(len(edge_index[0])))
...
def subgraph(data):
    ...
    adj = torch.zeros((node_num, node_num))
    adj[edge_index[0], edge_index[1]] = 1
    # print('[info] idx_sub is:{}'.format(len(idx_sub)))
    adj[idx_nondrop, :] = 0
    adj[:, idx_nondrop] = 0
